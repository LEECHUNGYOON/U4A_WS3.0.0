...
module.exports = function(sAppData, oAPPInstance){
    ...
    return new Promise(async function(resolve){
        ...
        //busy ON 처리된경우 exit.
        //다른 액션을 수행중(예 : 서버에 app 데이터 저장 요청)인 상황에서
        //AI를 통한 UI 데이터를 수신 받게 되는 경우
        //UI 데이터를 통한 화면 구성을 하지 않도록 처리.
        if(parent.getBusy() === "X"){
            
            _sRes.RETCD = "E";

            //$$MSG
            _sRes.RTMSG = "처리중인 작업이 존재하여 UI 구성을 진행할 수 없습니다.";
...
debugger;
        ...
        //호출된 팝업이 존재하는경우 EXIT.
        //다른 작업을 위해 팝업이 열려져 있는 경우(예 : UI 추가 팝업)
        //해당 액션을 우선 적으로 처리 해야 하기에 AI를 통한 UI 데이터를 수신 받게 되는 경우
        //UI 데이터를 통한 화면 구성을 하지 않도록 처리.
...
function changeRootUIObject(sParams){
    ...
    //최상위 라인 정보 얻기.
    var _indx = sParams.LIBDATA.T_0014.findIndex( item => item.POBID === "" );
    ...
    //최상위 UI 정보 얻기.
    let _s0014 = sParams.LIBDATA.T_0014[_indx];
    ...
    sParams.LIBDATA.T_0014.splice(_indx, 1);
    ...
    //     //AI를 통해 입력받은 ATTR 항목에 해당하는  ATTR수집건이 존재하는지 확인.
    //     var _indx = oAPP.attr.prev[_sAPP.OBJID]._T_0015.findIndex( item => item.UIATT === _s0015.UIATT && item.UIATY === _s0015.UIATY );

    //     //기존 ATTR 수집건이 존재하는경우 삭제 처리.
    //     if(_indx !== -1){
    //         oAPP.attr.prev[_sAPP.OBJID]._T_0015.splice(_indx, 1);
    //     }
    ...
    for (let i = 0, l = _aChild.length; i < l; i++) {
        ...
        _sChild.POBID = sParams.ROOT.OBJID;
...
function setRootEmbeddedAggregation(oUi, oLibMeta, oAggr) {
    ...
    _sAttr.OBJID = oUi.OBJID;
    _sAttr.UIATK = oAggr.UIATK;
    _sAttr.UIATT = oAggr.UIATT;
    _sAttr.UIASN = oAggr.UIASN;
    _sAttr.UIATY = "6";
    _sAttr.UIADT = oAggr.UIADT;
    _sAttr.ISMLB = oAggr.ISMLB;
    _sAttr.ISEML = "X";
    _sAttr.ISEMB = "X";

    _sAttr.UIOBK = oLibMeta?.UIOBK || "";
    _sAttr.UILIK = oLibMeta?.UILIK || "";
...
async function createUiFromTrasnport(sAppData){
    ...
    //선택한 라인이 최상위 ROOT인경우.
    if(_sParent.OBJID === CS_ROOT.ROOT_ID){
        //최상위 APP 정보를 검색.
        _sParent = oAPP.fn.getTreeData(CS_ROOT.ROOT_UI);
    }


    //선택한 라인이 ROOT APP일때의 UI 추가 처리.
    if(_sParent.OBJID === CS_ROOT.ROOT_UI){
...
debugger;
    ...
    var _isRemoved = removeRootAppAIData(sAppData);
     ...
     //부모가 없는 UI 얻기.
    var _s0014 = sAppData.T_0014.find( item => item.POBID === "" );

    //최상위 UI가 제거된 경우.
    if(_isRemoved === true){
        //ROOT가 된 CHILD UI중 embed aggregation이 제거된 UI를 얻기.
        _s0014 = sAppData.T_0014.find( item => item.POBID === "" && item.UIATT === C_REMOVE_ROOT.AGRNM );
    }
    ...
    //aggregation 선택 팝업에서 선택할 aggregation이 없거나, 팝업을 종료 처리 한경우.
    if(_sResAggr.RETCD === "E"){
        _sRes.RETCD = _sResAggr.RETCD;
        _sRes.RTMSG = _sResAggr.RTMSG;
    ...
    //부모 라인을 선택 처리.
    await oAPP.fn.setSelectTreeItem(sParent.OBJID); 
...
debugger;
    ...
    if(_found !== -1){
        ...
        //     PRCCD : "CHANGE_ATTR",
        //     RAND : _randHist,
        //     HIST : oAPP.attr.prev[sParent.OBJID]._T_0015
        // };
    ...
    //테스트주석처리!!!!!!!!!!!!!!!
    // //선택한 AGGREGATION의 CHILD에 UI가 존재하지 않는경우.
    // if(_found === -1){
...
async function createUiFromDrop(sAppData){
    ...
    //AI를 통해 전달받은 UI의 최상위 UI가 제거된 경우.
    if(_isRemoved === true){
        //ROOT가 된 CHILD UI중 embed aggregation이 제거된 UI를 얻기.
        _s0014 = sAppData.T_0014.find( item => item.POBID === "" && item.UIATT === C_REMOVE_ROOT.AGRNM );
    }
