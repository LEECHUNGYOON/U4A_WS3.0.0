...
(function (window, oAPP) {
	...
	oAPP.fn.callOTRListPopup = function (I_SHLPNAME, I_SHLP_DEF, IT_SHLP, IT_FIELDDESCR, f_clientCallbak) {
		...
		function lf_setSearchCondition(SHLPNAME, it_fdesc) {
			...
			for (var i = 0, l = it_fdesc.length; i < l; i++) {
				...
				switch (it_fdesc[i].DATATYPE) {
					case "D":
						// DATE TYPE인경우.
						var oSFld = new sap.m.DatePicker({
					...
					case "T":
						// TIME인경우.
						var oSFld = new sap.m.TimePicker({
							valueFormat: "HHmmss",
					...
					default:
						// DEFAULT INPUT필드.
						var oSFld = new sap.m.Input({
							type: "Text",
							valueLiveUpdate: true,
							maxLength: it_fdesc[i].OUTPUTLEN
						});

						//enter event
						oSFld.attachSubmit(function () {
							LF_getServerData();
						});
						break;
				}
		...
		var oPage = new sap.m.Page({
			title: "※ 결과리스트를 더블클릭하여 Alias를 복사할 수 있습니다."
		});
		oApp.addPage(oPage);
		...
		var SerchOVtoolbar = new sap.m.OverflowToolbar({
			width: "100%"
		});
		...
		var ZF4SH_ovtoolbar = new sap.m.OverflowToolbar({
			width: "100%"
		});

		//Maximum No, of Hits label.
		var olb01 = new sap.m.Label({
			design: "Bold",
		...
		//max Hits
		var ZF4SH_input01 = new sap.m.Input({
			type: "Number",
			width: "60px"
		});
		...
		//결과 건수 text
		var ZF4SH_LBresult = new sap.m.Label({
			design: "Bold",
